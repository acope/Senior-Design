...
public class jsscTest{
    ...
    public static void main(String[] args) {
        old.SerialComm arduino = new SerialComm();  
        
        arduino.detectPort();
        
        String ports[] = arduino.getPorts();
        
        for(String temp: ports){
            System.out.println(temp);
        }
        if(arduino.totalPorts() > 0){
            arduino.disconnectArduino();
            boolean success = arduino.connectArduino(ports[0]);
            System.out.println(success);
            if (success == true){
                arduino.disconnectArduino();
            }
        }else{
            System.out.println("There are no ports connected");
        }
    }
}
